...
document.addEventListener('DOMContentLoaded', function () {
  ...
  }, 200); //  seconds delay and redirect to login
  });
});
});

document.addEventListener('DOMContentLoaded', function () {
  const clickableLinks = document.querySelectorAll('.clickable-link2');
  clickableLinks.forEach(function (link) {
    link.addEventListener('click', function (event) {
      event.preventDefault();
...
document.addEventListener('DOMContentLoaded', function () {
  const sendButton = document.querySelector('.button input[type="button"]');
  // Add a click event listener
  sendButton.addEventListener('click', function () {
    // Show an alert message when the button is clicked
    alert('Message sent!');
    // Reload the page
    window.location.href = 'AboutContact.html';
  });
});
